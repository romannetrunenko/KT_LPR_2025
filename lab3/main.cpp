...
int main(int argc, char* argv[])
{
  ...
  constexpr double len_x = 100.0;
  constexpr double len_y = 100.0;
  
  {
    // Create mesh and function space
    auto part = mesh::create_cell_partitioner(mesh::GhostMode::shared_facet);
    auto mesh = std::make_shared<mesh::Mesh<U>>(
        mesh::create_rectangle<U>(MPI_COMM_WORLD, {{{0.0, 0.0}, {len_x, len_y}}},
                                  {256, 128}, mesh::CellType::triangle, part));
        ...
        *mesh, 1,
        [](auto x)
        {
          ...
          for (std::size_t p = 0; p < x.extent(1); ++p)
          {
            auto x0 = x(0, p);
            auto y0 = x(1, p);
            if (std::abs(x0) < eps or std::abs(x0 - len_x) < eps
                or std::abs(y0) < eps or std::abs(y0 - len_y) < eps)
              marker[p] = true;
          }
          return marker;
        });
    ...
    f->interpolate(
        ...
        {
          constexpr double q1_x = (len_x * 0.5) - 5;
          constexpr double q2_x = (len_x * 0.5) + 5;
          constexpr double inverse_charge_dispersion = 10.0;
          constexpr double charge_amplitude = 1000.0;
          std::vector<T> f;
          for (std::size_t p = 0; p < x.extent(1); ++p)
          {
            auto x0 = x(0, p);
            ...
            auto rsq1 = (x0 - q1_x) * (x0 - q1_x) + (y0 - (0.5*len_y))*(y0 - (0.5*len_y));
            auto rsq2 = (x0 - q2_x) * (x0 - q2_x) + (y0 - (0.5*len_y))*(y0 - (0.5*len_y));
            
            f.push_back(charge_amplitude*(std::exp(-rsq1 * inverse_charge_dispersion) - std::exp(-rsq2 * inverse_charge_dispersion)));
          }
